...
public function sendBOL($id)
{
    ...
    public function store(StoreShipmentRequest $request)
    {
        ...
        'po_number' => $po_num,
            ...
            'ocean_shipper_address' => $validatedData['ocean_shipper_address'] ?? null,
            'ocean_shippment_type' => $validatedData['ocean_shippment_type'] ?? null,
            'ocean_estimated_loading_date' => $validatedData['ocean_estimated_loading_date'] ?? null,
            'ocean_shipment_line' => $validatedData['ocean_shipment_line'] ?? null,
    ...
    private function storeLocations($shipment, $locations, $type)
    {
        if (!empty($locations['location_name']) && is_array($locations['location_name'])) {
            foreach ($locations['location_name'] as $index => $locationName) {
                $shipment->allLocations()->create([
                    'type' => $type,
                    'sequence' => $index + 1,
                    'pick_up_type' => $type === 'pickup' ? $locations['pick_up'][$index] ?? 'N/A' : 'N/A',
                    'drop_at_type' => $type === 'dropoff' ? $locations['drop_at'][$index] ?? 'N/A' : 'N/A',
